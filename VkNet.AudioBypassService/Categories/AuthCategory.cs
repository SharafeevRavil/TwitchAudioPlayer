...
public partial class AuthCategory : IAuthCategory
{
    ...
    public AuthCategory(IVkApiInvoke apiInvoke, IRestClient restClient, IVkTokenStore tokenStore,
    ...
    public Task<AuthValidateAccountResponse> ValidateAccountAsync(string login, bool forcePassword = false,
        bool passkeySupported = false, IEnumerable<LoginWay> loginWays = null)
    {
        return _apiInvoke.CallAsync<AuthValidateAccountResponse>("auth.validateAccount", new VkParameters
        {
            { "login", login },
    ...
    public Task<AuthValidatePhoneResponse> ValidatePhoneAsync(string phone, string sid, bool allowCallReset = true,
        IEnumerable<LoginWay> loginWays = null)
    {
        return _apiInvoke.CallAsync<AuthValidatePhoneResponse>("auth.validatePhone", new VkParameters
        {
            { "phone", phone },
            { "sid", sid },
            { "supported_ways", loginWays },
    ...
    {
        return await _apiInvoke.CallAsync<AuthCodeResponse>("auth.getAuthCode", new VkParameters
        {
    ...
    {
        return await _apiInvoke.CallAsync<AuthCheckResponse>("auth.checkAuthCode", new VkParameters
        {
            { "auth_hash", authHash },
            { "client_id", 7913379 },
            { "anonymous_token", _anonToken ?? await GetAnonTokenAsync() }
    ...
    {
        var response = await _apiInvoke.CallAsync<AuthRefreshTokensResponse>("auth.refreshTokens", new VkParameters
        {
    ...
    {
        return _apiInvoke.CallAsync<ExchangeTokenResponse>("execute.getUserInfo", new VkParameters
        {
            { "func_v", 30 },
            { "androidVersion", 32 },
    ...
    {
        var response = await _restClient.PostAsync(new Uri("https://api.vk.com/oauth/passkey_begin"), new VkParameters
        {
            { "sid", sid },
    ...
    private async ValueTask<string> GetAnonTokenAsync()
    {
        ...
        var response =
            await _restClient.GetAsync(new Uri(url), ImmutableDictionary<string, string>.Empty, Encoding.UTF8);
