...
public interface IIdentifiable
{
    ...
    {
        return enumerable?.IntersectBy(ids, static b => b.Identifier) ?? [];
    }

    private static IEnumerable<T> IntersectById<T>(IEnumerable<T>? enumerable, IEnumerable<long> ids)
        where T : IIdentifiable
    {
        return enumerable?.IntersectBy(ids, static b => long.Parse(b.Identifier)) ?? [];
    }

    private static IEnumerable<T> IntersectById<T>(IEnumerable<T>? enumerable, IEnumerable<int> ids)
        where T : IIdentifiable
    {
        return enumerable?.IntersectBy(ids, static b => int.Parse(b.Identifier)) ?? [];
    }
    ...
    {
        return enumerable?.IntersectBy(ids.Select(b => int.Parse(b.Identifier)), static b => int.Parse(b.Identifier)) ??
               [];
    }
    ...
    private static void ProcessBlock(Block block, ResponseData data)
    {
        ...
        foreach (var recommendedPlaylist in block.RecommendedPlaylists)
        {
            ...
            recommendedPlaylist.Playlist =
                IntersectById(data.Playlists, new[] { ((IIdentifiable)recommendedPlaylist).Identifier })
                    .FirstOrDefault()!;
        }

        foreach (var audio in block.Audios) audio.ParentBlockId = block.Id;
    }

    private static void ProcessSection(Section section, ResponseData data)
    {
        foreach (var block in section.Blocks) ProcessBlock(block, data);
