...
public class PlayerService
{
    ...
    protected readonly IEnumerable<ITrackMediaSource> _mediaSources;
    protected readonly TrackCollection _tracks = [];

    protected readonly Logger logger;
    protected readonly MediaPlayer player;
    protected int _nextPlayTrackIndex;
    protected DispatcherTimer _positionTimer;
    ...
    // protected readonly IEnumerable<ITrackStatsListener> _statsListeners;
    // protected readonly ConfigService _configService;

    protected int NextPlayTrackIndex
    {
    ...
    public IPlaylist? CurrentPlaylist { get; protected set; }
    ...
    public double Volume
    {
        ...
        set
        {
            ...
            player.Volume = value;
            VolumeChanged?.Invoke(this, player.Volume);
        }
    }
    ...
    public bool IsMuted
    {
        ...
        set
        {
            ...
            IsMutedChanged?.Invoke(this, player.IsMuted);
        }
    }
    ...
    public bool IsPlaying => player.PlaybackSession.PlaybackState is MediaPlaybackState.Playing
        or MediaPlaybackState.Opening or MediaPlaybackState.Buffering;
    ...
    public async void Play()
    {
        ...
        // await Task.WhenAll(_statsListeners.Select(b => b.TrackPlayStateChangedAsync(CurrentTrack!, player.Position, false)));
    }

    public async Task PlayTrackFromQueueAsync(int index, TimeSpan? startPosition)
    {
        // какое-то очередное говнище
        // var previousTrack = CurrentTrack!;
        // var previousPosition = player.Position;

        await PlayTrackAsync(index, startPosition);

        // await Task.WhenAll(_statsListeners.Select(b => b.TrackChangedAsync(previousTrack, CurrentTrack!, ChangeReason.TrackChange, previousPosition)));
    }
    ...
    protected async ValueTask<PlaylistTrack?> TrackAtOrDefaultAsync(int trackIndex)
    {
        ...
        switch (CurrentPlaylist)
        {
            ...
            case IRandomAccessPlaylist randomAccessPlaylist:
                ...
                _tracks.AddRangeSequential(
                    await randomAccessPlaylist.GetRangeAsync(
                        Math.Max(_tracks.Count, 0)..Math.Min(count, trackIndex + 10), this));
                break;
            default:
            {
                while (_tracks.Count - 1 < trackIndex && CurrentPlaylist.CanLoad)
                    _tracks.AddRangeSequential(await CurrentPlaylist.LoadAsync(this).ToArrayAsync());
    ...
    public virtual async Task PlayAsync(IPlaylist playlist, int? firstIndex = null, TimeSpan? startPosition = null)
    {
        if (!playlist.CanLoad)
        ...
        try
        {
            ...
            {
                await PlayTrackFromQueueAsync(firstIndex ?? CurrentIndex, startPosition);
                return;
            }

            CurrentPlaylist = playlist;
            CurrentPlaylist.TrackAdded -= CurrentPlaylistOnTrackAdded;
            CurrentPlaylist.TrackAdded += CurrentPlaylistOnTrackAdded;

            _tracks.Clear();
            CurrentIndex = -1;

            // _tracks.AddRangeSequential(playlist.GetCached()); // idk which one is right here
            _tracks.AddRange(playlist.GetCached());
            if (Tracks.Count > 0)
            {
                await PlayTrackFromQueueAsync(firstIndex ?? 0, startPosition);
                return;
            }
            ...
            if (firstIndex.HasValue)
            {
                ...
                if (firstTrack is null)
                {
                    ...
                    _tracks.AddRange(await CurrentPlaylist.LoadAsync(this).ToArrayAsync(),
            ...
            Application.Current.Dispatcher.BeginInvoke(
                () => QueueLoadingStateChanged?.Invoke(this, new PlayerLoadingEventArgs(PlayerLoadingState.Started)));

            if (firstTrackTask is null)
                _tracks.ReplaceRange(await playlist.LoadAsync(this).ToArrayAsync());
            else
            ...
            if (CurrentTrack is null)
            {
                ...
                // await Task.WhenAll(_statsListeners.Select(b => b.TrackChangedAsync(null, CurrentTrack!, ChangeReason.PlaylistChange, previousPosition)));
            }
        }
        catch (Exception e)
        {
        ...
        {
            Application.Current.Dispatcher.BeginInvoke(
                () => QueueLoadingStateChanged?.Invoke(this, new PlayerLoadingEventArgs(PlayerLoadingState.Finished)));
        }
    }

    private void CurrentPlaylistOnTrackAdded(object? sender,
        (object? initiator, IEnumerable<PlaylistTrack> newTracks) e)
    {
        if (!ReferenceEquals(CurrentPlaylist, sender)) return;
        if (e.initiator == this) return;

        _tracks.AddRangeSequential(e.newTracks);
    }

    protected async Task UpdateWindowsData()
    {
        try
        {
            ...
            if (CurrentTrack.AlbumId?.BigCoverUrl is not null)
                player.SystemMediaTransportControls.DisplayUpdater.Thumbnail =
                    RandomAccessStreamReference.CreateFromUri(new Uri(CurrentTrack.AlbumId.BigCoverUrl));
    ...
    public async Task NextTrack()
    {
        try
        {
            ...
            if (nextTrack is null)
            {
                ...
                if (nextIndex == Tracks.Count - 1 && CurrentPlaylist?.CanLoad == true) await LoadMore();
            }
            ...
            // await Task.WhenAll(_statsListeners.Select(b => b.TrackChangedAsync(previousTrack, nextTrack, ChangeReason.NextButton, previousPosition)));
        }
        catch (Exception ex)
        {
    ...
    public async void Seek(TimeSpan position, bool sync = false)
    {
        try
        {
            ...
            // await Task.WhenAll(_statsListeners.Select(b => b.TrackPlayStateChangedAsync(CurrentTrack!, position, !IsPlaying)));
        }
        catch (Exception e)
        {
    ...
    public async Task PreviousTrack()
    {
        try
        {
            ...
            else
            {
                ...
                // await Task.WhenAll(_statsListeners.Select(b => b.TrackChangedAsync(prevCurrentTrack, previousTrack!, ChangeReason.PrevButton, previousPosition)));
            }
        }
        catch (Exception e)
        {
    ...
    protected void MediaPlayerOnCurrentStateChanged(MediaPlaybackSession sender, object args)
    {
        try
        {
    ...
    protected async void MediaPlayerOnMediaFailed(MediaPlayer sender, MediaPlayerFailedEventArgs args)
    {
        if (args.Error == MediaPlayerError.SourceNotSupported)
        {
            ...
            {
                logger.Error("Mp3 boom not supported");
                /*try
                {
    ...
    protected async void PositionTimerOnTick(object sender, object o)
    {
        Application.Current.Dispatcher.Invoke(() => { PositionTrackChangedEvent?.Invoke(this, Position); });
    }
    ...
    public async void RemoveFromQueue(int index)
    {
        ...
        if (index == CurrentIndex + 2 && await TrackAtOrDefaultAsync(index) is not null) NextPlayTrackIndex = index;
    }
    ...
    protected void InitWindowsComamnds()
    {
        try
        {
    ...
    protected class TrackCollection : TrackCollectionBase
    {
        protected List<PlaylistTrack>? _originalTracks;
        protected List<int>? _shuffledIndexes;
        ...
        public void ShuffleTracks(int? startingIndex)
        {
            ...
            _shuffledIndexes = new List<int>(shuffledIndexes);
