...
public class GeniusService
{
    ...
    public GeniusService()
    {
        ...
        _client.DefaultRequestHeaders.TryAddWithoutValidation("User-Agent",
            "Genius/4.2.1 (Android; Android 14; Pixel 7 Pro)");
    }

    public async Task<IEnumerable<Hit>> SearchAsync(string query)
    {
        var (_, response) =
            await _client.GetFromJsonAsync<GeniusResponse<GeniusSearchResponse>>(
                $"search?q={string.Join('+', query.Split(' '))}");
    ...
    {
        var (_, response) =
            await _client.GetFromJsonAsync<GeniusResponse<GeniusSongResponse>>($"songs/{id}?text_format=plain");
