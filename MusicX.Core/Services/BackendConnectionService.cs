...
public class BackendConnectionService(Logger logger, string appVersion)
{
    ...
    public async void ReportMetric(string eventName, string? source = null)
    {
        try
        {
            ...
            using var response = await Client.PostAsJsonAsync($"/metrics/{eventName}/report",
                new ReportMetricRequest(appVersion, source));
            ...
            using (var httpClient = new HttpClient())
            {
                var response =
                    await httpClient.GetAsync(
                        "https://fooxboy.blob.core.windows.net/musicx/ListenTogetherServers.json");
