...
public class VkService
{
    ...
    public VkService(Logger logger, IVkApiCategories vkApi, IVkApiInvoke apiInvoke, IVkApiVersionManager versionManager,
        ...
        IDeviceIdStore deviceIdStore, ITokenRefreshHandler tokenRefreshHandler, IRestClient restClient)
    {
        this.vkApi = vkApi;
    ...
    public async Task<ResponseData> GetPlaylistAsync(int count, long albumId, string accessKey, long ownerId,
        int offset = 0, int needOwner = 1)
    {
        try
        {
    ...
    public async Task AddPlaylistAsync(long playlistId, long ownerId, string accessKey)
    {
        ...
        {
            logger.Info(
                $"audio.followPlaylist' with playlistId = '{playlistId}' and ownerId = '{ownerId}' and accessKey = {accessKey} ");
    ...
    public async Task<ResponseData> AudioGetAsync(long? playlistId, long? ownerId, string? assessKey, long offset = 0,
        long count = 100, int? shuffleSeed = null)
    {
        try
        {
            logger.Info(
                $"invoke 'audio.get' with playlistId = '{playlistId}' and ownerId = '{ownerId}' and accessKey = {assessKey} ");
            ...
            if (playlistId != null) parameters.Add("playlist_id", playlistId);

            if (ownerId != null) parameters.Add("owner_id", ownerId);

            if (assessKey != null) parameters.Add("access_key", assessKey);

            if (shuffleSeed != null) parameters.Add("shuffle_seed", shuffleSeed);

            var model = await apiInvoke.CallAsync<ResponseData>("audio.get", parameters);
            logger.Info("Successful invoke 'audio.get' ");
            var vkResponse = await apiInvoke.CallAsync<CountResponseVk>("audio.get", parameters);
            Console.WriteLine();
            var vkResponse2 = await apiInvoke.CallAsync<CountResponseVk>("audio.get", parameters);
            Console.WriteLine();
            return model;
    ...
    public async Task<int> AudioGetCountAsync(long? playlistId, long? ownerId)
    {
        try
        {
            logger.Info($"invoke 'audio.get' to .count with playlistId = '{playlistId}' and ownerId = '{ownerId}'");
    ...
    {
        var code =
            "var catalogs = API.catalog.getAudio({\"need_blocks\": 0}).catalog.sections;return API.catalog.getSection({\"need_blocks\": 1, \"section_id\": catalogs[0].id});";
    ...
    public async Task<long> CreatePlaylistAsync(long ownerId, string title, string description,
    ...
    public async Task EditPlaylistAsync(long ownerId, int playlistId, string title, string description,
        List<Audio> tracks)
    {
        try
        {
    ...
    public async Task<BoomToken> GetBoomToken()
    {
        ...
        try
        {
            ...
            if (model != null) model.Uuid = uuid.ToString();
    ...
    public async Task<List<Audio>> GetStreamMixAudios(string mixId = "common", int append = 0, int count = 5,
        ImmutableDictionary<string, ImmutableArray<string>>? options = null)
    {
        try
        {
            var parameters = new VkParameters
            {
                ...
                { "append", append },
                { "count", count },
                { "options", options == null ? null : JsonSerializer.Serialize(options, _jsonSerializerOptions) }
            };
    ...
    public async Task<CredentialResponse> GetMiniAppCredentialToken(string sourceUrl, string scope)
    {
        ...
        try
        {
            var response = await _restClient.PostAsync(new Uri("https://api.vk.com/oauth/authorize"), parameters,
                Encoding.UTF8);
