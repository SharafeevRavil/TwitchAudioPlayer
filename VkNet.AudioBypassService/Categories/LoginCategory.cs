...
public class LoginCategory : ILoginCategory
{
    ...
    public async Task ConnectAsync(string uuid)
    {
        ...
        var response = await _restClient.PostAsync(new Uri("https://login.vk.com/?act=connect_internal"), parameters,
            Encoding.UTF8);
    ...
    public async Task ConnectAuthCodeAsync(string token, string uuid)
    {
        ...
        var response = await _restClient.PostAsync(new Uri("https://login.vk.com/?act=connect_code_auth"), parameters,
            Encoding.UTF8);

        if (!response.IsSuccess)
            throw new HttpRequestException("Error connecting auth code", null, response.StatusCode);

        var (type, loginResponse) =
            JsonSerializer.Deserialize<LoginResponse<LoginResponseAccessToken>>(response.Value, _jsonSerializerOptions);
